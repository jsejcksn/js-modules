...
function getPlaceStatus (place, getEventList, unixTimestamp = Date.now()) {
  ...
  for (let i = 0; i < 7; i++) {
    days.push(events.filter(event => event.day === i));
  }
  ...
  // eventually replace days.reduce(...) with days.flat()
  for (const event of days.reduce((acc, val) => acc.concat(val), [])) {
    events.push(event);
  }

  const nowIndex = events.findIndex(event => event.type === 'now');
  let status;
  if (nowIndex > 0) {
  ...
  if (getEventList) {
    result.events = events.filter(event => !event.computed && event.type !== 'now');
  }

  return result;
}

export default getPlaceStatus;
